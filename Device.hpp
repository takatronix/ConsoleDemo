...
public:
    ...
    static DeviceOrientation getOrientation() {
        ...
        if (az > threshold) return FaceUp;
        if (az < -threshold) return FaceDown;
    ...
    static DeviceOrientation detectOrientation(float ax, float ay, float az) {
        ...
        if(Device::isStick() || Device::isAtom()){
            if (ax > threshold) return LandscapeRight;
            if (ax < -threshold) return LandscapeLeft;
            if (ay > threshold) return Portrait;
            if (ay < -threshold) return PortraitUpsideDown;
            return Unknown;            
        }
        
        // M5Stack系
        if (ax > threshold) return PortraitRight;
        if (ax < -threshold) return PortraitLeft;
        if (ay > threshold) return Landscape;
        if (ay < -threshold) return LandscapeUpsideDown;
    ...
    static bool isStick(){
        return M5.getBoard() == m5::board_t::board_M5StickC 
        || M5.getBoard() == m5::board_t::board_M5StickCPlus 
        || M5.getBoard() == m5::board_t::board_M5StickCPlus2;
    }
    static bool isAtom(){
        return M5.getBoard() == m5::board_t::board_M5Atom 
        || M5.getBoard() == m5::board_t::board_M5AtomPsram 
    ...
    static void viberate(int ms = 100){
        if(!Device::hasVibe)
            return;
        viberate(true);
        delay(ms);
